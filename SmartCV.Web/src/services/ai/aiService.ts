import type { AIMessage, AIProviderType, OptimizationResult, OptimizationSuggestion } from '../../types/ai';
...
export async function optimizeResume(
  ...
  let parsed: Omit<OptimizationResult, 'suggestions'> & { suggestions: Omit<OptimizationSuggestion, 'id' | 'applied'>[] };
  try {
...
function formatResumeAsText(resume: Resume): string {
  ...
  if (education.length > 0) {
    ...
    education.forEach(edu => {
      lines.push(`${edu.degree} in ${edu.field} - ${edu.institution} (${edu.startDate} - ${edu.current ? 'Present' : edu.endDate ?? ''})`);
