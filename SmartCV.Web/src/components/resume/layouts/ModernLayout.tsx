...
export function ModernLayout({ r, theme }: LayoutProps) {
  ...
  const renderSection = (key: string) => {
    switch (key) {
      case 'summary':
        return summary ? (
          <ModernSection key="summary" title="Summary" accent={accent}>
            <p style={{ color: '#374151', textAlign: 'justify', whiteSpace: 'pre-wrap' }}>{summary}</p>
          </ModernSection>
        ) : null;
      case 'coreHighlights':
        return coreHighlights?.length > 0 ? (
          <ModernSection key="coreHighlights" title="Core Highlights" accent={accent}>
      ...
      case 'experience':
        return experience.length > 0 ? (
          <ModernSection key="experience" title="Work Experience" accent={accent}>
            {experience.map(exp => (
      ...
      case 'education':
        return education.length > 0 ? (
          <ModernSection key="education" title="Education" accent={accent}>
            {education.map(edu => (
      ...
      case 'skills':
        return skills.length > 0 ? (
          <ModernSection key="skills" title="Professional Skills" accent={accent}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              {skills.map(s => (
      ...
      case 'projects':
        return projects.length > 0 ? (
          <ModernSection key="projects" title="Projects" accent={accent}>
            {projects.map(p => (
